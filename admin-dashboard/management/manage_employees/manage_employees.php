...
$result = $database->query("SELECT `id`, `username`, `email`, `user_type`, `location_code` FROM Users WHERE `status`= 'active'");
...
echo '<th onclick="sortTable(4)">Location Code</th>';
echo '<th>Action</th>';
echo '</tr>';
echo '</thead>';
echo '<tbody>';
...
while ($row = $result->fetch_assoc()) {
    ...
    $location_code = htmlspecialchars($row['location_code'], ENT_QUOTES, 'UTF-8');
    $id = htmlspecialchars($row['id'], ENT_QUOTES, 'UTF-8');

    // Ensure first_name and last_name are safely encoded
    ...
    echo '<td>' . $location_code . '</td>';
    echo '<td><button class="delete-button" data-id="' . $id . '">🗑️</button></td>';
    echo '</tr>';
}
