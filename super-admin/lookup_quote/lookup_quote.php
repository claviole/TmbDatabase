...
function generateExcelItemsFormHTML(quoteId) {
    ...
    if (quoteId.includes('QuickQuote')) {
        items = ['Part#', 'Blank Weight(lb)', 'Pcs Weight(lb)', 'ship_to_location', 'Blanking per piece cost', 'Packaging Per Piece Cost', 'freight per piece cost','material_cost','Total Cost per Piece'];
    } else {
        items =[
    'supplier_name',    
    'Part#',
