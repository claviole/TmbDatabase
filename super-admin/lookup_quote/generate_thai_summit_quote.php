...
$result = $database->query("SELECT * FROM invoice WHERE invoice_id = '{$invoice_id}' ORDER BY version DESC LIMIT 1");
...
$html .= '<tr><th style ="text-align:center;font-weight:bold" bgcolor="#ADD8E6">% Scrap Return to TSK</th><td style ="text-align:center;">' . $item['Scrap Consumption'] . '</td></tr>';
    ...
    $html .= '<tr><th style ="text-align:center;font-weight:bold" bgcolor="#ADD8E6">Wash and Lube Cost</th><td style ="text-align:center;">' . '$'. number_format($item['wash_and_lube']/$item['Pieces per Truck'],3) . "/Pc". '</td></tr>';
    $html .= '<tr><th style ="text-align:center;font-weight:bold" bgcolor="#ADD8E6">Material Cost</th><td style ="text-align:center;">' . '$'. number_format($item['material_cost_markup'],3) . "/Pc". '</td></tr>';
    $html .= '<tr><th style ="text-align:center;font-weight:bold" bgcolor="#ADD8E6">Total : </th><td style ="text-align:center;" bgcolor="#61FF33">' . '$'. number_format($item['freight per piece cost']+$item['Blanking per piece cost']+ $item['Packaging Per Piece Cost']+$item['wash_and_lube']/$item['Pieces per Truck']+$item['material_cost_markup'],3) . "/Pc". '</td></tr>';
