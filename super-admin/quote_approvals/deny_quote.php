...
if(isset($_POST['quoteId']) && isset($_POST['version'])){
    ...
    $stmt = $database->prepare("UPDATE `invoice` SET `approval_status` = 'Denied', `approved_by` = ? WHERE `invoice_id` = ? AND `version` = ?");
$stmt->bind_param("sss", $_SESSION['user'], $quoteId, $version);
    ...
    'message' => 'Quote Denied Successfully'
]);
}
} else {
    echo json_encode([
    'success' => false, // or false in case of an error
    'message' => 'Failed to deny quote'
]);
}
?>