...
async function addPart() {
	try {
		...
		if (window.partData) {
			...
			var partsPerHour =
				document.getElementById("pph").value * uptime_multiplier;
			...
			var blankWeightKg = parseFloat(
				(width / 1000) * (pitch / 1000) * gauge * Density
			).toFixed(3);
			...
			var wash_and_lube_choice =
				document.getElementById("wash_and_lube").checked;
			var material_cost;
			var wash_and_lube;
			...
			var material_markup_percent =
				document.getElementById("material_markup_percent").value / 100;
			var material_cost_markup = parseFloat(
				material_cost * material_markup_percent + material_cost
			).toFixed(3);
			...
			var blankingPerPieceCost = parseFloat(hourlyRate / partsPerHour).toFixed(
				3
			);
			var packagingPerPieceCost = parseFloat(
				palletCost / pcsPerSkid + 25 / pcsPerSkid
			).toFixed(3);
			var proccessingAndPackagingCost =
				blankingPerPieceCost + packagingPerPieceCost;
			var freightPerPiece = total_freight / pcsPerTruck;
			var totalPerPiece = (
				parseFloat(blankingPerPieceCost) +
				parseFloat(packagingPerPieceCost) +
				parseFloat(freightPerPiece) +
				parseFloat(wash_and_lube) / pcsPerTruck +
				parseFloat(material_cost_markup)
			).toFixed(3);
			...
			var cells = [
				...
				skidsPerTruck,
				weightPerTruck,
				blankingPerPieceCost,
				packagingPerPieceCost,
				totalPerPiece,
			];
...
function openExcelItemsPopup() {
	Swal.fire({
		...
		preConfirm: () => {
			...
			var selectedItemNames = Array.from(
				document.querySelectorAll('input[name="excel-item"]:checked')
...
async function submitInvoice() {
	...
	if (result.success) {
		...
		if (result.success) {
			...
			console.log(
				"No files or invoice_id received. Continuing to generate PDFs."
			);
		} else {
			// Handle other errors
